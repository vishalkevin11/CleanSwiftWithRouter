...
def xcodeproj = 'CleanSwiftWithRouter.xcodeproj' // Path to the xcodeproj
def xcarchive_name = "CleanSwiftWithRouter.xcarchive" // Name of the archive to build
def build_scheme = 'CleanSwiftWithRouter' // Scheme to build the app
def test_scheme = 'CleanSwiftWithRouter' // Scheme to build tests
        ...
        stage('Build') {
            wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'XTerm']) {
                ...
                dir('build') {
                    ...
                    sh "mv 'CleanSwiftWithRouter.ipa' 'CleanSwiftWithRouter-${branchNameForURL}.ipa'"
                }*/
            }
        }
        ...
        stage('Test') {
            wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'XTerm']) {
                ...
                if (sh(script: "which bluepill || true", returnStdout: true).trim().length() > 0) {
                    ...
                    parallel (
                        "Unit Tests": {
                            sh "bluepill -a '${app_path}' -s '${xcodeproj}/xcshareddata/xcschemes/CleanSwiftWithRouterTests.xcscheme' -o build/reports-tests -S 180 -T 450 -n 1 || true"
                        },
                        "UI Tests": {
                            sh "bluepill -a '${app_path}' -s '${xcodeproj}/xcshareddata/xcschemes/CleanSwiftWithRouterUITests.xcscheme' -o build/reports-uitests -S 180 -T 450 -n 1 || true"
                        }
                    )
                    ...
                    //sh "xcodebuild -scheme 'CleanSwiftWithRouter iPad' -enableCodeCoverage YES -configuration Debug -destination 'name=iPad Air 2' test | tee build/xcodebuild-ipad-test.log | xcpretty -r junit --output build/reports/junit-ipad.xml"
                    //sh "slather coverage --scheme 'CleanSwiftWithRouter iPad' --cobertura-xml --output-directory build/coverage '${xcodeproj}'"
